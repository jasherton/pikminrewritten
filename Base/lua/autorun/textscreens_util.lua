if SERVER then
	...
	hook.Add("InitPostEntity", "loadTextScreens", function()
		timer.Simple(10, function()
			...
			textscreens = file.Read("pik_textscreens.txt", "DATA")
			...
			for k, v in pairs(textscreens) do
				...
				local textScreen = ents.Create("pik_textscreen")
	...
	concommand.Add("SS_TextScreen", function(ply, cmd, args)
		...
		if not IsValid(ent) or ent:GetClass() ~= "pik_textscreen" then return false end
		...
		if args[1] == "add" then
			...
			file.Write("pik_textscreens.txt", util.TableToJSON(textscreens))
