...
local pikmintimehud = CreateClientConVar( "pikmintime_cl_hudeffects", 0, true, false );
local weathereffects = CreateClientConVar( "pikmintime_cl_weather", 0, true, false );
local RainRadius = CreateClientConVar( "pikmintime_cl_rainradius", 16, true, false );

pikmintimeStorming = false;
pikmintimeSnowing = false;

-- lightmap stuff
net.Receive( "pikmintime_lightmaps", function( len )
...
-- precache
hook.Add( "InitPostEntity", "pikmintimeFirstJoinLightmaps", function()
...
local pikmintimeRainSound = nil;
local pikmintimeThunderSound = nil;
local pikmintimeRainSoundPlaying = false;
local pikmintimeThunderSoundPlaying = false;
local pikmintimeRainSoundLastVolume = 0;
local HUDRainDrops = {};
...
local function StormThink()
	...
	-- rain sounds
	if ( pikmintimeRainSound == nil or !pikmintimeRainSoundPlaying ) then

		pikmintimeRainSound = CreateSound( LocalPlayer(), "atmos/rain.wav" );
		pikmintimeRainSound:PlayEx( 0, 100 );

		pikmintimeRainSoundPlaying = true;

	end

	if ( pikmintime_Outside( origin ) and pikmintimeRainSoundLastVolume != 0.4 ) then

		pikmintimeRainSound:ChangeVolume( 0.4, 1 );
		pikmintimeRainSoundLastVolume = 0.4;

	elseif ( !pikmintime_Outside( origin ) ) then

		if ( util.IsSkyboxVisibleFromPoint( origin ) and pikmintimeRainSoundLastVolume != 0.15 ) then

			pikmintimeRainSound:ChangeVolume( 0.15, 1 );
			pikmintimeRainSoundLastVolume = 0.15;

		elseif ( !util.IsSkyboxVisibleFromPoint( origin ) and pikmintimeRainSoundLastVolume != 0 ) then

			pikmintimeRainSound:ChangeVolume( 0, 1 );
			pikmintimeRainSoundLastVolume = 0;
	...
	if ( nextThunder < CurTime() ) then
		...
		pikmintimeThunderSound = CreateSound( LocalPlayer(), snd );
		pikmintimeThunderSoundPlaying = true;

		if ( pikmintime_Outside( origin ) ) then

			pikmintimeThunderSound:PlayEx( 1, 100 );

		else

			pikmintimeThunderSound:PlayEx( math.Rand( 0.3, 0.7 ), math.Rand( 60, 85 ) );
...
hook.Add( "Think", "pikmintimeStormThink", function()

	if ( !IsValid( LocalPlayer() ) ) then return end

	if ( pikmintimeStorming and weathereffects:GetInt() > 0 ) then

		StormThink();

	else

		if ( pikmintimeRainSound and pikmintimeRainSoundPlaying ) then

			pikmintimeRainSound:FadeOut( 3 );
			pikmintimeRainSoundPlaying = false;
			pikmintimeRainSoundLastVolume = 0;

		end

		if ( pikmintimeThunderSound and pikmintimeThunderSoundPlaying ) then

			pikmintimeThunderSound:FadeOut( 3 );
			pikmintimeThunderSoundPlaying = false;
	...
	if ( pikmintimeSnowing and weathereffects:GetInt() > 0 ) then
...
hook.Add( "HUDPaint", "pikmintimeHUDPaint", function()

	if ( !IsValid( LocalPlayer() ) ) then return end
	if ( render.GetDXLevel() <= 90 ) then return end
	if ( LocalPlayer():InVehicle() or LocalPlayer():WaterLevel() > 1 ) then return end
	if ( pikmintimehud:GetInt() < 1 or weathereffects:GetInt() < 1 ) then return end

	local origin, angles = LocalPlayer():EyePos(), LocalPlayer():EyeAngles();

	if ( pikmintimeStorming and pikmintime_Outside( origin ) and angles.p < 15 ) then
...
net.Receive( "pikmintime_storm", function( len )

	pikmintimeStorming = net.ReadBool();

end );

net.Receive( "pikmintime_snow", function( len )

	pikmintimeSnowing = net.ReadBool();

end );

net.Receive( "pikmintime_message", function( len )
...
local function UpdateValues( CPanel )
	...
	if ( CPanel.enabled ) then
		...
		CPanel.enabled:SetValue( cvars.Number( "pikmintime_enabled" ) );

	end

	if ( CPanel.paused ) then

		CPanel.paused:SetValue( cvars.Number( "pikmintime_paused" ) );

	end

	if ( CPanel.realtime ) then

		CPanel.realtime:SetValue( cvars.Number( "pikmintime_realtime" ) );

	end

	if ( CPanel.weather ) then

		CPanel.weather:SetValue( cvars.Number( "pikmintime_weather" ) );

	end

	if ( CPanel.snowenabled ) then

		CPanel.snowenabled:SetValue( cvars.Number( "pikmintime_snowenabled" ) );

	end

	if ( CPanel.length_day ) then

		CPanel.length_day:SetValue( cvars.Number( "pikmintime_dnc_length_day" ) );

	end

	if ( CPanel.length_night ) then

		CPanel.length_night:SetValue( cvars.Number( "pikmintime_dnc_length_night" ) );

	end

	if ( CPanel.weatherchance ) then

		CPanel.weatherchance:SetValue( cvars.Number( "pikmintime_weather_chance" ) );

	end

	if ( CPanel.weatherlength ) then

		CPanel.weatherlength:SetValue( cvars.Number( "pikmintime_weather_length" ) );

	end

	if ( CPanel.weatherdelay ) then

		CPanel.weatherdelay:SetValue( cvars.Number( "pikmintime_weather_delay" ) );

	end

end

--[[local function pikmintimeSettings( CPanel )
...
concommand.Add( "pikmintime_cl_reset", function( pl, cmd, args )

	RunConsoleCommand( "pikmintime_cl_hudeffects", 1 );
	RunConsoleCommand( "pikmintime_cl_weather", 1 );

	RunConsoleCommand( "pikmintime_cl_rainsplash", 1 );
	RunConsoleCommand( "pikmintime_cl_rainclouds", 1 );
	RunConsoleCommand( "pikmintime_cl_rainradius", 16 );
	RunConsoleCommand( "pikmintime_cl_rainperparticle", 16 );
	RunConsoleCommand( "pikmintime_cl_maxrainheight", 180 );
	RunConsoleCommand( "pikmintime_cl_raindietime", 5 );

	RunConsoleCommand( "pikmintime_cl_snowdietime", 6 );
	RunConsoleCommand( "pikmintime_cl_snowradius", 1200 );
	RunConsoleCommand( "pikmintime_cl_snowperparticle", 20 );
	RunConsoleCommand( "pikmintime_cl_maxsnowheight", 300 );

	print( "pikmintime clientside settings reset.\n" );
