...
function ENT:Initialize()
	self:SetModel("models/pikmin/pikmin_yellow1.mdl"); -- old model: models/props_junk/PlasticCrate01a.mdl
...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("pik_pluckyellow");
	ent:SetPos(SpawnPos);
	ent:Spawn();
	ent:Activate();
...
function ENT:Think()
	...
	self:SetModel("models/pikmin/pikmin_yellow2.mdl")
	...
	self:SetModel("models/pikmin/pikmin_yellow3.mdl")
	local effectdata = EffectData();
			local pos, ang;
...
if (self.pluckstages == 1) then
	...
	self:Remove();
	v:ConCommand( "pikmin_create yellow" );
else
--no please
end
else
...
if (self.pluckstages == 2) then
	...
	self:EmitSound("pikmin/burrowpull.wav", 100, math.random(98, 105));
	self:Remove();
	v:ConCommand( "pikmin_create_bud yellow" );
else
--no please
end
else
...
if (self.pluckstages == 3) then
	...
	self:EmitSound("pikmin/burrowpull.wav", 100, math.random(98, 105));
	self:Remove();
	v:ConCommand( "pikmin_create_flower yellow" );
else
--no please
end
else
